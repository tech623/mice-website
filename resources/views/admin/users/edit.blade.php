...
<div class="card card-primary">
    ...
    <div class="card-body">
        ...
        @endif
        <form method="POST" action="{{ route("panel.user-managment.update", [$user->id]) }}" enctype="multipart/form-data">
            @csrf
            @method("put")
            ...
            @can('is_admin')
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('role') ? 'is-invalid' : '' }}" multiple="multiple" name="role[]" id="role" required>
                        @foreach($roles as $id => $role)
                        <option value="{{ $role->id }}" {{ (in_array($role->id, old('role', [])) || $user->roles->contains($role->id)) ? 'selected' : '' }}>{{ $role->name }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('role'))
