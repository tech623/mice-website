...
<div class="card card-primary" style="width: fit-content;">
                ...
                <div class="form-group"  style="border-style:outset;">
                    <select name="event_id" id="event_id" class="form-control" value="{{request()->input('event_id')}}">
                        ...
                        @foreach($services as $service)
                            <option value='{{$service->id}}' @selected(request()->input('event_id') == $service->id)>{{$service->backend_name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>  
    ...
    <div class="row mb-4 mt-4">
        ...
        @if($filter > 0)
            <div class="col-md-2 ml-3">
                @if(request()->input('status'))
                    <strong style="color:gray;">Remove Status</strong> 
                    <a href="javascript:void(0);" title="Remove Status Filter" onclick="$('#status').empty();$('#filterform').submit();">
                    <i class="fas fa-backspace" style="font-size: 1.25em"></i>
                    </a>
                @endif
            </div>
        ...
        <div class="row">
                ...
                <tbody>
                    ...
                    @foreach ($enquiries as $enquiry)
                        <tr>
                            ...
                            <td>{{ $enquiry->dealdata->total_cost ?? "" }}</td>
                            <td>{{ $enquiry->dealdata->mice_revenue ?? "" }}</td>
            ...
            @endif
                {!! $enquiries->appends(Request::all())->links() !!}
        </div>
    </div>
