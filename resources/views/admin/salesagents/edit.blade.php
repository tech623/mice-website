...
<div class="card card-primary">
    ...
    <div class="card-body">
        ...
        @endif
        <form method="POST" action="{{ route("panel.sales-agent.update", [$user->id]) }}" enctype="multipart/form-data">
            @csrf
            @method("put")
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('supervisor') ? 'is-invalid' : '' }}" name="supervisor" id="supervisor" style="width: 100%;">
                    ...
                    <option value="{{$supervisor->id}}" {{ $supervisor->id == $user->superVisor->supervisor_id  ? "selected" : "" }}>{{$supervisor->name}}</option>
                    @endforeach
                </select>
